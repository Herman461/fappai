...
if(password&&password.classList.contains('has-no-error')&&password.value!==confirmedPassword.value){if(button){button.setAttribute('disabled','')}
return}
if(hasValidInputs&&!document.querySelector('.content-edit .errors-list')){if(button){button.removeAttribute('disabled')}}else{if(button){button.setAttribute('disabled','')}}}
...
if(photoWrapperElement){photoWrapperElement.appendChild(errorsList)}}
...
if(uploadButton){uploadButton.addEventListener('click',function(){avatarInput.click()})}
if(avatarInput){avatarInput.addEventListener('change',function(e){const avatar=e.currentTarget.files[0]
