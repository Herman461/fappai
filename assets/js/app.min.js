...
if(!video)return
const player=videojs(video.id);activeVideo=video
player.play()
lockActiveVideo=true
e.target.closest('.item-video').addEventListener('mouseleave',function(e){resetActiveVideo()
setTimeout(function(){lockActiveVideo=false},70)},{once:true})}}
function resetActiveVideo(){if(activeVideo){const player=videojs(activeVideo.id);player.hasStarted(false);player.pause();player.currentTime(0);activeVideo.closest('.item-video').classList.remove('active')
activeVideo=null}}
feed.addEventListener('touchstart',function(e){function detectOffset(e){if(!e.target.closest('.item-video'))return
const video=e.target.closest('.item-video').querySelector('video')
const player=videojs(video.id);if(e.target.closest('.item-video').classList.contains('active')){resetActiveVideo()
return}
e.preventDefault()
resetActiveVideo()
e.target.closest('.item-video').classList.add('active')
...
if(gridButton){gridButton.addEventListener('click',function(e){const items=document.querySelectorAll('.feed__items')
...
if(listButton){listButton.addEventListener('click',function(e){const items=document.querySelectorAll('.feed__items')
if(items.length>0){for(let index=0;index<items.length;index++){const item=items[index]
