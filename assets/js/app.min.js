...
menu.addEventListener('click',function(e){if(!e.target.closest('.menu__body')){toggleMenu()}})
searchHeaderInput.addEventListener('focus',function(e){if(menu.classList.contains('active')){toggleMenu()}
...
window.addEventListener('click',function(e){if(e.target.closest('.info-video__more')){e.target.closest('.info-video__text').classList.add('full')}
if(e.target.closest('.info-video__less')){e.target.closest('.info-video__text').classList.remove('full')}
if(e.target.closest('.tag__more')){e.target.closest('.tags').classList.toggle('full')}})
const feed=document.querySelector('.feed')
if(feed){feed.addEventListener('mousemove',debounce(playVideo,20))}
let lockActiveVideo=false
function playVideo(e){if(lockActiveVideo)return
if(e.target.closest('.item-video')&&!e.target.closest('.item-video').classList.contains('active')){const video=e.target.closest('.item-video').querySelector('video')
e.target.closest('.item-video').classList.add('active')
if(!video)return
const player=videojs(video.id);player.play()
lockActiveVideo=true
e.target.closest('.item-video').addEventListener('mouseleave',function(e){player.hasStarted(false);player.pause();player.currentTime(0);e.target.closest('.item-video').classList.remove('active')
setTimeout(function(){lockActiveVideo=false},100)},{once:true})}}
feed.addEventListener('touchstart',function(e){function detectOffset(e){if(!e.target.closest('.item-video')||e.target.closest('.item-video').classList.contains('active'))return
const video=e.target.closest('.item-video').querySelector('video')
const player=videojs(video.id);if(e.target.closest('.item-video').classList.contains('active')){player.hasStarted(false);player.pause();player.currentTime(0);e.target.closest('.item-video').classList.remove('active')}
e.preventDefault()
e.target.closest('.item-video').classList.add('active')
player.play()}
e.currentTarget.addEventListener('touchmove',debounce(detectOffset,50),{once:true})})
window.addEventListener('click',function(e){if(document.querySelector('.item-video.active')&&!e.target.closest('.item-video.active')){const video=document.querySelector('.item-video.active video')
const player=videojs(video.id);player.hasStarted(false);player.pause();player.currentTime(0);video.closest('.item-video.active').classList.remove('active')}})
const gridButton=document.querySelector('.view-feed__item_grid')
const listButton=document.querySelector('.view-feed__item_list')
gridButton.addEventListener('click',function(e){const items=document.querySelectorAll('.feed__items')
...
listButton.addEventListener('click',function(e){const items=document.querySelectorAll('.feed__items')
if(items.length>0){for(let index=0;index<items.length;index++){const item=items[index]
